...
namespace WebForm_upload
{
    public partial class _Default : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                //處理按鈕click的各種情況
                if (!string.IsNullOrEmpty(Request.Form["BtnFirstPage"])) ///第一頁
                {
                    Session["pageIndex"] = 1;
                    BtnFirstPage.Enabled = false;
                    BtnPPage.Enabled = false;
                ...
                else if (!string.IsNullOrEmpty(Request.Form["BtnPPage"])) ///上一頁
                {
                ...
                else if (!string.IsNullOrEmpty(Request.Form["BtnNPage"])) ///下一頁
                {
                    int page = (int)Session["pageIndex"];
                    page++;
                    Session["pageIndex"] = page;
                ...
                else if (!string.IsNullOrEmpty(Request.Form["BtnLastPage"])) ///最後頁
                {
        ...
        private void GetFileList()
        {
            ...
            if (Directory.Exists(filePath))
            {
                ...
                uploadFiles.Columns.Add("Name");

                string[] files = Directory.GetFiles(filePath);
                for (int i = 0; i < files.Length; i++)
                {
                    FileInfo fInfo = new FileInfo(files[i]);
                    DataRow row = uploadFiles.NewRow();
                    row["Name"] = fInfo.Name;
                    uploadFiles.Rows.Add(row);
                }

                FileList.DataSource = uploadFiles;
                FileList.DataBind();
            }
        }
    }
